...
class DataLoaderMultiProcess:
    ...
    def generate_train_batch(self):
        ...
        for j, key in enumerate(selected_keys):
            ...
            # print(data.shape)#(4, 155, 240, 240)
            this_slice = tuple([slice(0, data.shape[0])] + [slice(i, j) for i, j in zip(valid_bbox_lbs, valid_bbox_ubs)])
            # print(seg.shape)#得是(1, 155, 240, 240)
            data = data[this_slice]

            this_slice = tuple([slice(0, seg.shape[0])] + [slice(i, j) for i, j in zip(valid_bbox_lbs, valid_bbox_ubs)])
            # 将 seg 的形状改为 (1, 155, 240, 240)
            if seg.ndim == 3:
                seg = seg[np.newaxis, :]
            seg = seg[this_slice]# 必须要(1, 155, 240, 240)，但是我的原始数据忘记改了，是 (155, 240, 240)
    ...
    def get_bbox(self, data_shape, force_fg, class_locations, overwrite_class=None, verbose=False):
        # 找出最小维度作为深度
        sorted_dims = sorted(enumerate(data_shape), key=lambda x: x[1])
        depth_idx = sorted_dims[0][0]
        other_indices = [sorted_dims[1][0], sorted_dims[2][0]]

        # 设置裁剪大小
        patch_size = [0, 0, 0]
        patch_size[depth_idx] = 80
        patch_size[other_indices[0]] = 160
        patch_size[other_indices[1]] = 160
        ...
        # 按中心裁剪
        bbox_lbs = [max(0, (data_shape[i] - patch_size[i]) // 2) for i in range(dim)]
