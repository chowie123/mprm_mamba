...
from muti_mamba.light_training.utils.lr_scheduler import get_constant_schedule_with_warmup, get_cosine_schedule_with_warmup, \
...
class Trainer:
    ...
    def get_multi_processor_loader(self, train_ds, val_ds):
        ...
        from muti_mamba.light_training.dataloading.base_data_loader import DataLoaderMultiProcess
    ...
    def validation_single_gpu(self, val_dataset, ):
        ...
        for idx, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            with torch.no_grad():
                val_out,hd95 = self.validation_step(batch)
        ...
        if self.global_step % self.val_every == 0 \
            ...
            if self.ddp:
                ...
                for i in range(len(outputs_split)):
                    ...
                    hd95_outs = torch.tensor(hd95_split[i]).cuda(self.local_rank)
                    val_outputs_merge.append(
                        distributed_concat(val_outputs, num_total_examples=len(self.val_loader) * self.num_gpus))
                    hd95_outs_merge.append(
                        distributed_concat(hd95_outs, num_total_examples=len(self.val_loader) * self.num_gpus))
                ...
                for i in range(len(hd95_split)):
                    val_outputs = torch.tensor(hd95_split[i])
                    hd95_outs_merge.append(val_outputs)
                # val_outputs = torch.tensor(val_outputs)

            if self.local_rank == 0:
                if len(val_outputs_merge) == 1:
                    val_outputs_merge = val_outputs_merge[0]
                self.validation_end(val_outputs_merge,hd95_outs_merge)
    ...
    def train(self,
        ...
        if self.scheduler_type == "cosine_with_warmup":
            ...
            from muti_mamba.light_training.utils.lr_scheduler import PolyLRScheduler
                ...
                if not self.auto_optim:
                    ...
                    with autocast("cuda", enabled=True) if (self.ddp or 'cuda' in self.device) else dummy_context():
                        if self.print_time:
                            s = time.time()
                        loss = self.training_step(batch)
                        # loss = self.training_step(batch,len(self.train_loader))
                    ...
                    if self.grad_scaler is not None:
                        ...
                        #         total_gradient += torch.norm(param.grad, p=2).item()
                        # self.gbr_scores.append(total_gradient)
    ...
    def test(self,
        ...
        # if self.scheduler_type == "cosine_with_warmup":#没用
        #     if self.warmup == 0.0:
        #         self.warmup = 0.1
        #     assert self.warmup < 1 and self.warmup > 0
        #     warmup_steps = self.max_steps * self.warmup
        ...
        #                                                      num_training_steps=self.max_steps)
        #     print(f"warmup steps is {warmup_steps}")
        # elif self.scheduler_type == "constant_with_warmup":#没用
        #     if self.warmup == 0.0:
        #         self.warmup = 0.1
        #     assert self.warmup < 1 and self.warmup > 0
        #     warmup_steps = self.max_steps * self.warmup
        ...
        #     print(f"warmup steps is {warmup_steps}")
        #
        # elif self.scheduler_type == "poly_with_warmup":#没用
        #     if self.warmup == 0.0:
        #         self.warmup = 0.1
        #     assert self.warmup < 1 and self.warmup > 0
        #     warmup_steps = self.max_steps * self.warmup
        ...
        #     print(f"warmup steps is {warmup_steps}")
        #
        # elif self.scheduler_type == "poly":#没用
        #     from muti_mamba.light_training.utils.lr_scheduler import PolyLRScheduler
        #     lr = self.optimizer.state_dict()['param_groups'][0]['lr']
        #     print(f"initial lr is {lr}")
        #     self.scheduler = PolyLRScheduler(self.optimizer, initial_lr=lr, max_steps=self.max_steps)
