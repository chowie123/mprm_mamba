...
class Mamba_muti(nn.Module):
    def __init__(
        ...
        self.in_proj3 = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
        ...
        self.x_proj2 = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        )
        self.x_proj3 = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        )
        self.dt_proj = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)
        self.dt_proj1 = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)
        self.dt_proj2 = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)
        self.dt_proj3 = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)
        ...
        A_log2 = torch.log(A2)  # Keep A_log in fp32
        A_log3 = torch.log(A3)  # Keep A_log in fp32
        ...
        self.D2 = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
        self.D2._no_weight_decay = True

        self.D3 = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
    ...
    def forward(self, hidden_states,hidden_states_1,hidden_states_2,hidden_states_3, inference_params=None):
        """
        hidden_states: (B, L, D)
        ...
        xz1 = rearrange(
            self.in_proj1.weight @ rearrange(hidden_states_1, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        )
        xz2 = rearrange(
            self.in_proj2.weight @ rearrange(hidden_states_2, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        )
        xz3 = rearrange(
            self.in_proj3.weight @ rearrange(hidden_states_3, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        )
        if self.in_proj.bias is not None:
            xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")
        if self.in_proj1.bias is not None:
            xz1 = xz1 + rearrange(self.in_proj1.bias.to(dtype=xz.dtype), "d -> d 1")
        if self.in_proj2.bias is not None:
            xz2 = xz2 + rearrange(self.in_proj2.bias.to(dtype=xz.dtype), "d -> d 1")
        if self.in_proj3.bias is not None:
            xz3 = xz3 + rearrange(self.in_proj3.bias.to(dtype=xz.dtype), "d -> d 1")
        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)
        A1 = -torch.exp(self.A_log1.float())  # (d_inner, d_state)
        A2 = -torch.exp(self.A_log2.float())  # (d_inner, d_state)
        A3 = -torch.exp(self.A_log3.float())  # (d_inner, d_state)
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            if conv_state is not None:
                ...
                conv_state.copy_(x1[:, :, -self.d_conv :])  # Update state (B D W)
                conv_state.copy_(x2[:, :, -self.d_conv :])  # Update state (B D W)
                conv_state.copy_(x3[:, :, -self.d_conv :])  # Update state (B D W)

            if causal_conv1d_fn is None:
                x = self.act(self.conv1d(x)[..., :seqlen])
                x1 = self.act(self.conv1d(x1)[..., :seqlen])
                x2 = self.act(self.conv1d(x2)[..., :seqlen])
                x3 = self.act(self.conv1d(x3)[..., :seqlen])
            ...
            x_dbl1 = self.x_proj1(rearrange(x, "b d l -> (b l) d"))  # (bl d)
            x_dbl2 = self.x_proj2(rearrange(x, "b d l -> (b l) d"))  # (bl d)
            x_dbl3 = self.x_proj3(rearrange(x, "b d l -> (b l) d"))  # (bl d)

            dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            dt1, B1, C1 = torch.split(x_dbl1, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            dt2, B2, C2 = torch.split(x_dbl2, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            dt3, B3, C3 = torch.split(x_dbl3, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            ...
            dt1 = self.dt_proj1.weight @ dt1.t()
            ...
            dt2 = self.dt_proj2.weight @ dt2.t()
            ...
            dt3 = self.dt_proj3.weight @ dt3.t()
            dt3 = rearrange(dt3, "d (b l) -> b d l", l=seqlen)
            B3 = rearrange(B3, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            C3 = rearrange(C3, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            assert self.activation in ["silu", "swish"]
            y = selective_scan_fn(
                x,
                dt,
            ...
            y1 = selective_scan_fn(
                ...
                self.D1.float(),
                z=z1,
                delta_bias=self.dt_proj1.bias.float(),
                delta_softplus=True,
                return_last_state=ssm_state is not None,
            )
            y2 = selective_scan_fn(
                ...
                self.D2.float(),
                z=z2,
                delta_bias=self.dt_proj2.bias.float(),
                delta_softplus=True,
                return_last_state=ssm_state is not None,
            )
            y3 = selective_scan_fn(
                ...
                self.D3.float(),
                z=z3,
                delta_bias=self.dt_proj3.bias.float(),
                delta_softplus=True,
                return_last_state=ssm_state is not None,
            )
